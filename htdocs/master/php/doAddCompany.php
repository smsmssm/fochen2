...
$check_sql="select Femail from tcompany where Femail='".$mail."'";//是否已经有用户注册
$result = mysql_query($check_sql);
$row = mysql_fetch_array($result, MYSQL_NUM);

if($row[0]==$mail&&$row[0]!="45169132@qq.com")
{
   mysql_close($link);
...
{
    mysql_query($sql);
	$check_sql="select Fname,id,Femail from tcompany where Femail='".$mail."'";//是否
    $result = mysql_query($check_sql);
    $row = mysql_fetch_array($result,MYSQL_NUM);
   // mysql_close($link);
