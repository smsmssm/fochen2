...
function api_send_mail($email,$subject,$content){
	...
	$smtppass = "sanmao123";//SMTP服务器的用户密码
...
function api_send_a_mail($sender,$email,$subject,$content){
	$smtpserver = "smtp.ym.163.com";//SMTP服务器
	$smtpserverport =25;//SMTP服务器端口
	$smtpusermail = $sender;//"admin@byu4218000001.my3w.com";//SMTP服务器的用户邮箱
	$smtpemailto = $email;//发送给谁
	$smtpuser = $sender;//"admin@byu4218000001.my3w.com";//SMTP服务器的用户帐号
	$smtppass = "smsmssm123";//SMTP服务器的用户密码
	$mailsubject = $subject;//邮件主题
	$mailbody = $content;//邮件内容
	$mailtype = "HTML";//邮件格式（HTML/TXT）,TXT为文本邮件
	$smtp = new smtp($smtpserver,$smtpserverport,true,$smtpuser,$smtppass);//这里面的一个true是表示使用身份验证,否则不使用身份验证.
 ...
 function setMenu()
 {
    ///array_push($menu,"学佛入门");
	$obj  = array();
    $row  = array("认识佛学","因果感应","戒杀放生","佛教故事","学佛常见问答");
	$row2 = array("rsfx","yggy","jsfs","fjgs","cjwd");
	
	array_push($obj,array("name" => "学佛入门","path" =>"xfrm", "subMenu" => $row,"subPath" =>$row2));
	
	
	$row  = array("皈依受戒","早课晚课","念佛","参禅","念经诵咒","放生护生","布施施食","忏悔","修持方法");
	$row2 = array("gysj","zkwk","nf","cc","njsz","fshs","bsss","ch","xcff");
	array_push($obj,array("name" => "在家修行","path" =>"zjxx", "subMenu" => $row,"subPath" =>$row2));
...
function getMenuName($index,$subMenuIndex,$type)
{
	...
	if($type=="main")return $menuObj[$index]["name"]; //返回一级菜单名
	else  if($type=="sub")return $menuObj[$index]["subMenu"][$subMenuIndex];      //返回二级菜单名
